...
| Derivation/edge/lineage/candidate/replay/revocation/quarantine/waiver/rebuild/verification/attestation endpoints (spec 8.2) | missing | the Service-mode SQLAlchemy tables for these (Phase A) still have no repository/writer or API surface; `revocation.execute` exists only as a job handler (Phase C2 above), not an API-triggerable workflow yet |
