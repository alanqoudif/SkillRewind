...
| Revocation/rebuild/verification/attestation endpoints (spec 8.2) | missing | `revocation.execute` exists only as a job handler (Phase C2), not an API-triggerable workflow yet; a replay-confirmed edge does not itself trigger revocation (Phase C2.2 invariant 4: that must stay a separate, explicit action); this is the next backend gate |
